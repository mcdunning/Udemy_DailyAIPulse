...
- **Design doc before code.** Every feature planning/brainstorming session must produce a written design doc under `docs/superpowers/specs/` that the user has explicitly approved before any code is written — even for changes that would otherwise look small or "bounded." If the work is for an existing feature, update that feature's existing spec doc rather than creating a new one; create a new doc only when the work is for a genuinely new feature. If the decision is cross-cutting/shared-architecture (affects multiple features, or a file the architecture doc already owns, e.g. `navigation/AppNavigation.kt` or `core/`), fold it directly into `docs/superpowers/specs/2026-09-12-architecture-design.md` (dated inline, e.g. "**Added YYYY-MM-DD**") rather than creating a standalone spec file for it.
...
- **Clean up git worktrees right after pushing.** Isolated git worktrees (e.g. via an `EnterWorktree`-style tool) are fine to use for multi-task automated implementation runs, but once the branch is pushed and a PR is opened, remove the worktree immediately — don't leave it parked for later PR-feedback iteration. A parked worktree keeps that branch checked out, which blocks checking it out anywhere else (e.g. in Android Studio) until the worktree is removed. If PR feedback later needs more commits, create a fresh worktree at that time.
- **Validate all reference documents whenever `docs/PROJECT_STATUS.md` is updated.** In the same change, check every other reference document — `README.md` and everything under `docs/superpowers/specs/` — for stale mentions of a feature's old status (e.g. "planned", "blank placeholder", "not yet built", a package-layout diagram or "## Feature Specs"/doc-index list missing the finished feature) and fix them — don't wait to be asked. A finished feature's docs should read as finished everywhere, not just in the status file. **Exclude `docs/superpowers/plans/`** — implementation plans are point-in-time task instructions describing state as of when they were written; they're historical artifacts, not living reference docs, and shouldn't be rewritten after the fact.
