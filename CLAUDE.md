...
- **Validate all reference documents whenever `docs/PROJECT_STATUS.md` is updated.** In the same change, check every other reference document — `README.md` and everything under `docs/superpowers/specs/` and `docs/superpowers/plans/` — for stale mentions of a feature's old status (e.g. "planned", "blank placeholder", "not yet built", a package-layout diagram or "## Feature Specs"/doc-index list missing the finished feature) and fix them — don't wait to be asked. A finished feature's docs should read as finished everywhere, not just in the status file.
